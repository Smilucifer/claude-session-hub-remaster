...
export function buildExportFilename(title: string): string {
  return `claw-go-${sanitizeFilename(title)}-${dateStr()}.html`;
}
...
<head>
  ...
  <meta name="generator" content="Claw GO">
  <title>${title} — Claw GO</title>
  <style>
${styles}
/* Export overrides */
...
<body class="bg-background text-foreground">
  ...
  <footer style="max-width:48rem;margin:0 auto;padding:1.5rem;text-align:center;font-size:0.75rem;color:hsl(var(--muted-foreground))">
    Generated by <a href="https://github.com/AnyiWang/ClawGO" style="color:hsl(var(--primary))">Claw GO</a>
  </footer>
  <script>
