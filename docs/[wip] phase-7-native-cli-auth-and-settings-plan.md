...
- **Task 3B follow-up progress - 2026-05-06:** native PTY turns now classify transcript results through explicit `Completed` / `Stopped` / `Failed` terminal states, preserve failure text unless a stop marker was explicitly set, persist matching run statuses for room polling, and surface `chat-done.error` back into the frontend session store.
- **Task 3B follow-up progress - 2026-05-06:** targeted regression coverage now exists for native transcript baseline reuse, stopped/failed room terminal outcomes, room failed-response payload propagation, frontend `handleChatDone(...)` failed/stopped behavior, and stricter Codex/Gemini repeated multi-turn baseline progression.
...
- This task is currently in progress in a parallel session using `D:\ClaudeWorkspace\Code\claude-session-hub-remaster\docs\superpowers\specs\2026-05-06-provider-native-connection-entry-design.md` as the active design reference.
- This session should avoid overlapping provider-entry code changes while that parallel work is active, and instead continue on non-conflicting native parity acceptance work.
