...
**Date:** 2026-04-30
**Status:** Active
**Purpose:** Track how each meaningful Claude Session Hub capability maps into the OpenCovibe remaster.
...
| Category | Meaning |
|----------|---------|
| Migrate | Rebuild the product protocol or UX natively in OpenCovibe. |
| Reference | Keep as design input, but do not port directly. |
| Drop | OpenCovibe already has a better native path, or the capability is no longer needed. |
...
| `renderer/renderer.js` (memo block) | Migrate | The Hub memo UX is the seed for OpenCovibe's three-tier memo (global / project / room). | `storage/memos.rs`, memo Svelte panel |
...
| `core/data-dir.js` | Migrated | Env-driven data-dir override is useful for isolated E2E tests and parallel dev runs. | `storage::data_dir()` with `OPENCOVIBE_DATA_DIR` |
...
|------------|--------|-----------|--------|
| Windows MSVC spawn environment resolver | Merged with Settings/status UI | Hub users often hit Windows native build failures when Claude/Codex child processes start outside Developer PowerShell. OpenCovibe should make local child CLI spawns usable from the normal desktop window. | `src-tauri/src/agent/windows_msvc_env.rs`, local spawn paths |
| Native Codex/Gemini chat parity | Merged on 2026-05-05 | Hub-style multi-agent workflows need Codex and Gemini to be usable as first-class chat seats, but OpenCovibe should keep native CLI pipe-exec semantics instead of adding a PTY wall. | `src-tauri/src/agent/stream.rs`, `src/lib/stores/session-store.svelte.ts`, `/chat`, Settings connection panels |

## Reference

| Hub Source | Decision | Rationale | Target |
|------------|----------|-----------|--------|
| `core/mobile-protocol.js` | Reference | PWA and Tailscale usage patterns are useful, but OpenCovibe already has a Rust web server. | Web remote UX notes |
| `core/deep-summary-service.js` | Reference | Summary orchestration may inform future room summaries. | Later summary backlog |
| `core/summary-providers/` | Reference | Provider fallback ideas are useful; implementation should follow OpenCovibe settings/provider architecture. | Later summary provider design |
...
| `core/usage-filter.js` | Reference | Monotonic filtering can inform future real-time usage badges; OpenCovibe's native stats path remains canonical. | Future usage UI polish |

## Drop

| Hub Source | Decision | Rationale | Replacement |
|------------|----------|-----------|-------------|
| `core/mobile-server.js` | Drop | OpenCovibe has `web_server` with axum, cookie/token auth, and WS broadcast. | `src-tauri/src/web_server` |
...
| `core/ansi-utils.js` | Drop | OpenCovibe already has frontend ANSI/xterm utilities. | Existing `src/lib/utils/ansi.ts`, xterm components |
| `core/state-store.js` | Drop | OpenCovibe has structured Rust storage. | `storage/*` |
| `~/.claude/scripts/session-hub-hook.py` (deployed by Hub) | Drop | OpenCovibe receives stream-JSON events from Claude CLI directly; no external hook script needed. | Existing event stream |
| `~/.claude/scripts/claude-hub-statusline.js` (deployed by Hub) | Drop | OpenCovibe computes context / usage from CLI events natively. | Existing usage tracking |
| `core/session-archive.js` | Drop | OpenCovibe already has native CLI session import/history paths; do not reintroduce Hub's `~/.claude/projects/*/*.jsonl` archive scanner. | `storage/cli_sessions.rs`, `commands/cli_sync.rs`, history/search UX |
| `core/lindang-bridge.js` | Drop | Product-specific A-share data bridge does not belong in OpenCovibe core. | Optional plugin/MCP integration if needed later |

## Non-Goal

No Hub module should be copied wholesale into OpenCovibe. Even migrated items are protocol and UX migrations, implemented in the native OpenCovibe architecture.
